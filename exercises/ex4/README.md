...
runs per cell (all runs solved the task; `results.txt` holds the raw logs).

| Scenario | Budget 1 s | Budget 20 s |
|----------|-----------|-------------|
| Single agent | RESULTS_SINGLE_1S | RESULTS_SINGLE_20S |
| Two robots | RESULTS_MULTI_1S | RESULTS_MULTI_20S |
...
multimodal — the effect is visible mainly in the two-robot scenario, where
the joint action space (9 actions per node) spreads the same simulation
budget much thinner, and coordinated behaviors (the same-cell heavy push)
need deep, consistent action sequences to show value. In the single-agent
scenario the problem is easy enough that 1 s is already near the plateau,
and 20 s mostly reduces variance rather than the mean.
...
larger than in the single-agent case; a large share of the extra steps is
spent on the rendezvous + coordinated-push phase. The second robot does not
